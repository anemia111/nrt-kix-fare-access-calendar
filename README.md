...
成田空港（NRT）と関西国際空港（KIX）を結ぶ便について、**対応航空会社の公式予約サイト・出発ターミナル・最寄り空港駅・駅からの移動時間・公式の搭乗締切・空港到着目標**を確認できる日本語Webアプリです。

**公開URL: https://anemia111.github.io/nrt-kix-fare-access-calendar/**

> [!IMPORTANT]
> **実用モード（`/`・`/calendar/`）は、架空データを一切表示しません。**
> 航空券の価格・空席・便の時刻は表示せず、公式に確認できる情報だけを扱います。
> 価格・空席は各航空会社の公式サイトでご確認ください。
>
> 機能の動きを確認するための**デモモード（`/demo/`・`/demo/calendar/`）**は分離して残してあり、
> デモの全画面上部に「機能確認用のデモです。価格・空席・列車時刻は実際の情報ではありません」を常時表示します。

実価格APIを利用できないため、サイト名から「最安値」を外しています。実価格APIが有効になり、
Peach とジェットスターを含む比較範囲を正確に説明できる場合にのみ「最安値」表記を有効化します
（`src/domain/siteConfig.ts` の `canClaimLowestFare()`）。
...
| --- | --- | --- |
| `/` | 実用 | 実用トップページ。架空データなし |
| `/calendar/` | 実用 | 出発計画（公式情報のみ） |
| `/demo/` | デモ | デモトップページ（常時警告つき） |
| `/demo/calendar/` | デモ | デモ最安値カレンダー（架空の価格・空席・列車時刻） |

### 実用モードで表示しないもの

架空の航空券価格 / 架空の空席数 / 架空の販売可否 / 架空の列車時刻 / 架空の列車運賃 / デモの価格再検証結果。

これは型レベルで保証しています。`kind: "demo"` の出所を持つ値は、実用モードのコンポーネントが
受け取る `ProductionValue<T>` に代入できません（`src/domain/provenance.ts`）。
テストでも `@ts-expect-error` により、デモデータを実用モードへ渡すとコンパイルエラーになることを検証しています。

### 実用モードの検索条件と表示項目

**入力**: 路線（成田→関空／関空→成田）・搭乗日・大人人数・預け荷物の有無・朝／昼／夜・出発駅・
（任意）出発時刻。

**表示**: 入力した日付・路線・対応航空会社・公式予約サイト・出発ターミナル・最寄り空港駅・
駅からターミナルまでの移動時間・公式搭乗締切・空港駅到着目標・前泊を検討すべきか・情報源・最終確認日。

> [!NOTE]
> 実在する便の時刻を取得できないため、**便の時刻はこちらで生成しません**。
> 空港駅到着目標は、利用者が公式サイトで確認した出発時刻を任意入力した場合にのみ計算します。
...
**Node.js 24 以上**が必要です（`package.json` の `engines.node` と `.nvmrc` に記載）。
GitHub Actions も Node 24（`actions/checkout@v6` / `actions/setup-node@v6` /
`actions/configure-pages@v6` / `actions/upload-pages-artifact@v5` / `actions/deploy-pages@v5` /
`actions/upload-artifact@v7` / `actions/github-script@v9`）で統一し、
Node.js 20 非推奨警告を解消しています。
...
npm run holidays:generate  # 内閣府CSVから祝日データを再生成
npm run sources:check      # 公式ページの変更を確認（監視のみ）
```
...
> [!IMPORTANT]
> **Google Flights / SerpApi は正確な残席数を提供しません。**
> したがって実データ検索でも「残り3席」のような表示は行いません。
> 通常は「現在予約候補あり」「予約可否は購入画面で最終確認」と表示します。
> これは API 仕様上の制限であり、実装で回避できるものではありません。
>
> 型でも保証しています（`SeatAvailability`）。`kind: "exact"` の場合だけ
> `remainingSeats` を持ち、`bookable` / `unknown` / `soldOut` は席数を保持できません。
> 「◯名まで検索可能」の類は残席数として扱いません。

以下はデモモードを含む、席数表示の一般方針です。
...
「公式サイトで価格・空席を確認」を押しても、**すぐには外部サイトへ移動しません**。
まず確認画面を表示し、航空会社・搭乗日・出発／到着空港・人数・片道・エコノミー・預け荷物条件と、
「公式サイトで再入力が必要であること」を示します。

ボタン: 検索条件をコピー / 日付をコピー / 便名をコピー（**実データの便名がある場合のみ**）/
公式サイトを開く / 閉じる。

正式に公開されたディープリンク仕様が存在しないため、日付・便名が選択済みであるかのようには見せず、
次を明示します。

> 航空会社が便指定ディープリンクを公開していないため、公式サイトで条件を再入力してください

デモの便名は実データではないため、デモモードでも「便名をコピー」は表示しません。

---

## 実データ検索（SerpApi + Google Routes）

実際の便・価格・公共交通経路を検索できます。**APIキーは GitHub Pages の
JavaScript には一切含めず**、Cloudflare Worker（無料枠）経由でのみ呼び出します。

```
ブラウザ（GitHub Pages・静的）
        │  fetch（キーを持たない）
        ▼
Cloudflare Worker  ──── SerpApi Google Flights（航空券）
（Secrets にキー）   └── Google Routes API（公共交通）
```

### 処理の流れ

1. 条件（路線・搭乗日・大人人数・預け荷物・時間帯・出発駅）を入力して検索
2. SerpApi から**実在する直行便**を取得し、対象航空会社（MM/GK/NH/JL/IJ）だけ表示
3. 便ごとに公式の搭乗締切から**空港駅到着目標**を算出（既存ロジック）
4. その時刻を `arrivalTime` に指定して Google Routes（TRANSIT）で経路検索
5. 既存ロジックで「遅くとも乗るべき経路」と「推奨経路」を選択
6. 「最新価格・予約可否を確認」で**再検索**し、同一便を複数条件で照合
7. 価格が変わっていれば確認ダイアログ（値上がり・値下がりとも表示）
8. `booking_token` があれば Booking Options を取得し購入導線へ

航空券結果を先に表示し、公共交通経路は後から差し込みます（全画面をブロックしません）。

### API エンドポイント（Cloudflare Worker）

| メソッド | パス | 内容 |
| --- | --- | --- |
| POST | `/api/flights/search` | 航空券検索（60分キャッシュ） |
| POST | `/api/flights/revalidate` | 再検索・価格比較（**キャッシュを使わない**） |
| POST | `/api/flights/booking-options` | Booking Options 取得 |
| POST | `/api/transit/search` | 公共交通経路（24時間キャッシュ・15分バケット） |
| GET | `/api/reference/boarding-rules` | 公式搭乗ルール |
| GET | `/api/reference/terminals` | ターミナル・駅情報 |
| GET | `/api/health` | 稼働確認（キーの有無のみ返す） |
...
```bash
npx wrangler login                                             # 要ログイン
npx wrangler secret put SERPAPI_API_KEY     --config worker/wrangler.toml
npx wrangler secret put GOOGLE_MAPS_API_KEY --config worker/wrangler.toml
npm run worker:deploy
```

デプロイ後に表示される URL を、フロントの `NEXT_PUBLIC_API_BASE_URL` に設定します
（この URL は秘密情報ではないため公開して構いません）。
`worker/wrangler.toml` の `ALLOWED_ORIGINS` に本番オリジンを入れてください。

#### 2. Google Cloud（Routes API）— コスト保護

1. プロジェクトを作成し、**Routes API だけ**を有効化する（他は無効のまま）
2. APIキーを作成し、**APIキー制限**で「Routes API」のみ許可する
3. HTTPリファラ制限は使わない（Worker からの呼び出しのためIP/無制限になる）。
   代わりに**キーをWorkerのSecretsだけに置く**ことで保護する
4. 「割り当て（Quota）」で1日あたりの上限を設定する
   - 目安: Routes API の Compute Routes を **1日 300 リクエスト**程度に制限すると、
     月間の無料クレジット（$200相当）を大きく下回ります
   - Routes API の TRANSIT は課金対象です。必ず上限を設定してください
5. 「予算とアラート」で少額（例: ¥1,000）の予算アラートを作成する

#### 3. SerpApi — 無料枠保護

- Free プランは**月100検索**です（2026-07 時点。最新はダッシュボードで確認）
- 本アプリは**ユーザーが検索した時だけ**APIを呼びます
- **毎日90日分を全件クロールするようなことはしません**（無料枠が即座に尽きるため）
- 同一条件は Worker 側で60分キャッシュし、繰り返し検索を上流へ流しません
- 残り検索数は SerpApi ダッシュボード（Account → Usage）で確認できます
- 上限に達した場合は「今月のリアルタイム航空券検索上限に達しました。
  航空会社公式サイトで確認してください」と表示し、**デモへは切り替えません**

### 無料枠を守るキャッシュ設計

| 対象 | TTL | キー |
| --- | --- | --- |
| 航空券検索 | 60分 | origin / destination / date / adults / cabin / directOnly |
| 公共交通経路 | 24時間 | 出発駅 / 到着駅 / 到着目標時刻（**15分バケット**） |
| 再検索（revalidate） | キャッシュなし | 予約前は必ずライブ取得 |

到着目標時刻が近い便は同じ経路検索結果を共有するため、便ごとに無制限に
Routes API を呼ぶことはありません。キャッシュキーに個人情報は含めません。
画面には取得時刻と「キャッシュ／ライブ取得」を表示します。
...
超過時は HTTP 429 と「検索回数が多すぎます。少し時間をおいて再度お試しください」。
IP が取得できない環境では User-Agent 等の弱い識別子を使い、全員を同一クライアント
として扱わないようにしています。

> [!NOTE]
> レート制限は Worker の isolate 内メモリで管理する best-effort 実装です
> （KV 名前空間を作らず無料枠のみで動かすため）。厳密なグローバル制限が必要に
> なった場合は KV / Durable Objects へ移行してください。

---

## 公式情報の日次監視（監視のみ・値の自動変更はしません）

GitHub Actions が毎日、公式ページの変更を検出します（`.github/workflows/monitor-sources.yml`、
`scripts/check-official-sources.mjs`）。

処理:

1. 各ホストの `robots.txt` を取得し、**許可されていないパスは対象外**にする
2. ETag・Last-Modified・本文の SHA-256 ハッシュを取得する
3. 前回値（`monitor/sources.state.json`）と比較する
4. 変更がなければ終了する（コミットもしない）
5. 変更があればレポートを生成し、GitHub Issue を作成する
6. **締切時間などの値は自動変更しない**（検出のみ。人手で確認して更新する）
7. 取得失敗時は last-known-good を維持する（既存の値を消さない）

Issue タイトル例: `公式情報の変更を検出: Peach 国内線 搭乗案内`
Issue 本文には 対象URL / 前回・今回の確認日時 / 前回・今回のハッシュ / HTTPステータス /
ETag / Last-Modified / 手動確認が必要な項目 を含めます。

### 監視対象URL

| 対象 | URL | 現在の状態 |
| --- | --- | --- |
| Peach 搭乗案内 | https://www.flypeach.com/lm/ai/airports/checkin | 取得不可（要手動確認） |
| Peach 成田空港案内 | https://www.flypeach.com/lm/ai/airports/airportguide_domestic/nrt | 取得不可（要手動確認） |
| Peach 関西空港案内 | https://www.flypeach.com/lm/ai/airports/airportguide_domestic/kix | 取得不可（要手動確認） |
| ジェットスター 空港手続きの締切 | https://www.jetstar.com/jp/ja/help/when-do-i-need-to-get-to-the-airport | 監視中 |
| ジェットスター 成田第3ターミナル | https://www.jetstar.com/jp/ja/help/nrt-t3 | 監視中 |
| ANA 国内線 搭乗手続き | https://www.ana.co.jp/ja/jp/guide/boarding-procedures/checkin/domestic/flow_airport/ | 監視中 |
| JAL 国内線 ご搭乗のお願い | https://www.jal.co.jp/jp/ja/dom/boarding_attention/ | 取得不可（要手動確認） |
| 成田空港 第3ターミナル移動 | https://www.narita-airport.jp/ja/access/train/railway-route-3/ | 監視中 |
| 関西空港 第2ターミナル連絡バス | https://www.kansai-airport.or.jp/access/t2 | 監視中 |
| 内閣府 国民の祝日 | https://www8.cao.go.jp/chosei/shukujitsu/gaiyou.html | 監視中 |
| 内閣府 祝日CSV | https://www8.cao.go.jp/chosei/shukujitsu/syukujitsu.csv | 監視中 |

**「取得不可」の4件**（Peach 3件・JAL 1件）は、自動リクエストに対して 2xx 以外を返すため
本文ハッシュを取得できません。**スクレイピングやアクセス制限の回避は行いません。**
これらは `fetch-failed` として記録され、last-known-good を維持したうえで
「手動確認が必要」として扱います。値の更新は人手で行ってください。

---

## 祝日データ（自動生成）

`src/data/holidays.generated.json` は**自動生成ファイルです。手動で編集しないでください。**

- 取得元: 内閣府「国民の祝日」CSV https://www8.cao.go.jp/chosei/shukujitsu/syukujitsu.csv
- 生成: `npm run holidays:generate`（`scripts/generate-holidays.mjs`）
- 定期実行: `.github/workflows/update-holidays.yml`（毎週月曜 06:40 JST）
- CSV は Shift_JIS のため `TextDecoder("shift_jis")` で正しくデコードします
- 日付・祝日名を検証し、不正な行は捨てます
- **振替休日・国民の休日も保持**します（CSV の表記どおり「休日」として格納）
- 差分（本文ハッシュ）がある場合だけ更新します
- **取得失敗時は既存ファイルを削除せず維持**します（last-known-good）
- 出典URL・取得日時・ハッシュ・ETag・Last-Modified を `_meta` に保持します
- **対応外の年は平日扱いにせず `unknown`** として安全側に計算します
...
| チェック | 結果 |
| --- | --- |
| TypeScript 型チェック | ✅ エラーなし（デモ→実用モードの型混入もここで検出） |
| ESLint | ✅ エラー0・警告0 |
| Vitest | ✅ 165件すべて通過 |
| Playwright | ✅ 34件すべて通過（iPhone 15/WebKit・Desktop Chrome） |
| 本番ビルド | ✅ 成功 |
| npm audit | ✅ 脆弱性0件 |

### 今回追加したテスト

- 実用モードにデモ価格・デモ空席・デモ便名が表示されない（E2E）
- デモモードに常時警告が表示される（E2E）
- 正式ディープリンクがない場合に「条件引き継ぎ済み」と表示しない（E2E）
- 便名コピーは実データの便名がある場合のみ表示（E2E）
- コピー機能とトースト表示（E2E / ユニット）
- URL共有と復元・不正値のフォールバック（ユニット / E2E）
- LocalStorage への保存と復元・重複排除（ユニット）
- 祝日CSVの変換（日付正規化・不正行の除去・振替休日の保持）
- 祝日取得失敗時の last-known-good 維持
- 公式ページの変更検出（ハッシュ変更 / 変更なし / 初回 / 取得失敗 / robots による除外）
- ETag を伴う変更検出、GitHub Issue 用レポート生成
- robots.txt の解析と Allow/Disallow の優先順位
- デモデータを実用モードへ渡すとコンパイルエラーになること（`@ts-expect-error`）
- 375px で横スクロールが発生しないこと（E2E）
- キーボード操作（Enter で確認画面を開き、Escape で閉じる）（E2E）
- アクセシビリティ属性（`aria-pressed` / `role="dialog"` / `aria-live`）
...
| 機能 | 実装 |
| --- | --- |
| 最近の検索条件を LocalStorage へ保存 | `src/lib/recentSearches.ts`（`useSyncExternalStore` で購読） |
| 検索条件をURLクエリへ保存・復元 | `src/domain/planSearch.ts` |
| URL共有 / Web Share API | `src/lib/share.ts`（非対応環境はクリップボードへフォールバック） |
| コピー成功時のトースト | `src/components/Toast.tsx`（`aria-live`つき） |
| 空港到着目標を ICS で保存 | `src/lib/ics.ts`（JST の VTIMEZONE 同梱） |
| 読み込み中のスケルトン | 各ページの Suspense フォールバック |
| 375px で横スクロールなし | E2Eで検証 |
| キーボード操作・フォーカス表示 | `:focus-visible` の明示スタイル、Escape で閉じる |
| `aria-live` による状態通知 | 検索結果見出し・トースト |
| `prefers-reduced-motion` 対応 | `globals.css` でアニメーション無効化 |
| データ区分を色以外でも表示 | 記号（✔ ◆ ⚠ —）＋文字ラベル（`PROVENANCE_SYMBOLS`） |
| PWA 対応 | `src/app/manifest.ts` + `public/sw.js`（basePath 対応） |
| オフライン表示 | 保存済みの公式基礎情報のみ表示し、最新でない旨を明示（`AppChrome.tsx`） |

> 月単位の遅延読み込みは、実用モードが「単一日の公式情報」を表示する構成になったため
> 対象がなくなりました。デモカレンダーは月単位でグループ化して描画しています。
...
| ワークフロー | 実行契機 | 内容 |
| --- | --- | --- |
| `ci.yml` | main への push / PR / 手動 | 型チェック・Lint・Vitest・ビルド・Playwright・npm audit |
| `deploy.yml` | main への push / 手動 | 静的エクスポートを GitHub Pages へデプロイ |
| `monitor-sources.yml` | **毎日 06:17 JST** / 手動 | 公式ページの変更検出と Issue 作成（監視のみ） |
| `update-holidays.yml` | **毎週月曜 06:40 JST** / 手動 | 内閣府CSVから祝日データを再生成 |

`monitor/**` のみの変更では CI と再デプロイを行いません（`paths-ignore`）。

## 将来の実価格API

インターフェース（`FlightProvider`）は維持しています。状態は
`src/domain/priceState.ts` に定義済みで、`realTimePrice` / `cachedReferencePrice` /
`bookingAvailable` / `availabilityUnknown` / `soldOut` / `providerUnavailable` /
`unsupportedCarrier` を表現できます。

> [!WARNING]
> **実データモード（live）に設定してもプロバイダーが未設定・無効な場合、デモへ自動フォールバックしません。**
> 架空データを本番情報として見せないためです。この場合は
> 「実価格検索プロバイダーが設定されていません」と表示します。

正確な残席数を取得できない場合は席数を表示しません（`availabilityUnknown`）。

## 現在の制限

- **実用モードは価格・空席・便の時刻を表示しません**（無料で取得できる提供元が無いため）
- **デモモードの価格・空席・列車時刻は架空データ**です（`/demo/` 配下に分離）
- 公式ページのうち Peach 3件・JAL 1件は自動取得に応答しないため、**監視は「取得失敗」として記録**され、
  内容確認は手動が必要です（スクレイピング・制限回避は行いません）
